...
import { useCallback, useEffect, useState, useSyncExternalStore, useMemo } from '@wordpress/element';
import { subscribe } from '@wordpress/data';
import { applyFilters } from '@wordpress/hooks';
import {
	AI_PREVIEW_PENDING_COUNT_EVENT,
	fetchAiPreviewPendingCount,
	formatAiPreviewPendingCount,
} from './ai-preview-pending-count';
...
function syncSettingsPendingBadge( button, count ) {
	if ( ! button ) {
		return;
	}

	const next = Math.max( 0, Number( count ) || 0 );
	let badge = button.querySelector( '.blockish-editor-settings-pending-badge' );

	if ( next > 0 ) {
		if ( ! badge ) {
			badge = document.createElement( 'span' );
			badge.className = 'blockish-editor-settings-pending-badge';
			badge.setAttribute( 'aria-hidden', 'true' );
			button.appendChild( badge );
		}
		badge.hidden = false;
		badge.textContent = formatAiPreviewPendingCount( next );
		button.setAttribute(
			'aria-label',
			sprintf(
				/* translators: %d: number of pending AI Preview designs */
				__( 'Settings, %d pending', 'blockish' ),
				next
			)
		);
		return;
	...
	button.setAttribute( 'aria-label', __( 'Settings', 'blockish' ) );
}
...
function EditorSettingsModal() {
	...
	const refreshPendingCount = useCallback( async () => {
		const count = await fetchAiPreviewPendingCount();
	...
	useEffect( () => {
		...
		const onCountEvent = ( event ) => {
			if ( typeof event?.detail?.count === 'number' ) {
				setPendingCount( Math.max( 0, event.detail.count ) );
				return;
			}
			refreshPendingCount();
		};
		...
		window.addEventListener( AI_PREVIEW_PENDING_COUNT_EVENT, onCountEvent );
		window.addEventListener( 'focus', onFocus );
		document.addEventListener( 'visibilitychange', onFocus );

		return () => {
			window.removeEventListener( AI_PREVIEW_PENDING_COUNT_EVENT, onCountEvent );
	...
	useEffect( () => {
		...
		if ( ! button ) {
			...
			button.innerHTML = `
                ...
                <span class="blockish-editor-settings-toolbar-label">${ __( 'Settings', 'blockish' ) }</span>
            `;
			const templateLibraryRoot = document.getElementById(
				'blockish-template-library-root'
			);
		...
		syncSettingsPendingBadge( button, pendingCount );

		return () => {
			button.removeEventListener( 'click', openModal );
			button.remove();
		};
	}, [ toolbarElement, tabs.length, refreshPendingCount ] );

	useEffect( () => {
		const button = document.getElementById(
			'blockish-editor-settings-toolbar-button'
		);
		syncSettingsPendingBadge( button, pendingCount );
	}, [ pendingCount ] );
	...
	return (
		<>
			<PluginMoreMenuItem icon={ settingsIcon } onClick={ () => setIsOpen( true ) }>
				...
				{ pendingCount > 0 ? ` (${ formatAiPreviewPendingCount( pendingCount ) })` : '' }
			</PluginMoreMenuItem>
			{ isOpen && (
				<Modal
					...
					<div className="blockish-settings-layout">
							...
							tabs={ tabs.map( ( tab ) => ( {
								...
								title: (
									<div className="blockish-settings-tab-header">
										<span className="blockish-settings-tab-title">
											{ tab.title }
											{ tab.name === 'ai-preview' && pendingCount > 0 && (
												<span className="blockish-settings-tab-badge">
													{ formatAiPreviewPendingCount( pendingCount ) }
												</span>
											) }
										</span>
										{ tab.description && (
											<span className="blockish-settings-tab-description">
												{ tab.description }
											</span>
										) }
									</div>
						...
						>
							{ ( tab ) => {
								...
								if ( ! activeTabConfig || ! activeTabConfig.render ) {
									return null;
								}
