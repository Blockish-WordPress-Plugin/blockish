...
const Inspector = ( { attributes, setAttributes, advancedControls } ) => {
	...
	const tagVal = getTagValue( attributes?.tag, 'div' );
	const isKnownTag = COMMON_WRAPPER_TAGS.includes( tagVal );
	...
	const handleSelectChange = ( selectedOption ) => {
		if ( ! selectedOption ) {
			setAttributes( { tag: { label: 'div', value: 'div' } } );
			return;
		}
		if ( selectedOption.value === 'custom' ) {
			setAttributes( {
				tag: { label: 'custom-wrapper', value: 'custom-wrapper' },
			} );
		} else {
			setAttributes( {
				tag: {
					label: selectedOption.label || selectedOption.value,
					value: sanitizeTagName( selectedOption.value, 'div' ),
				},
			} );
		}
	};
								...
								>
									...
									{ isCustom && (
										<BlockishControl
											...
											placeholder="e.g. section, custom-element"
