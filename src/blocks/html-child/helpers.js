...
/**
 * Common child / inline tags suitable for html-child.
 */
export const COMMON_CHILD_TAGS = [
	'span',
	'p',
	'button',
	'a',
...
export const TAG_OPTIONS = [
	...COMMON_CHILD_TAGS.map( ( tag ) => ( { label: tag, value: tag } ) ),
...
/**
 * Check if a tag is a void HTML element.
 *
 * @param {string} tag
 * @return {boolean}
 */
...
/**
 * Sanitize an HTML tag name.
 *
 * @param {string} tag
 * @param {string} fallback
 * @return {string}
 */
export function sanitizeTagName( tag, fallback = 'span' ) {
	...
	if ( ! cleaned || BLOCKED_TAGS.has( cleaned ) ) {
		return fallback;
	}
	return cleaned;
}
 ...
 * @return {string}
 */
export function getTagValue( tag, fallback = 'span' ) {
	if ( ! tag ) {
		return fallback;
	}
