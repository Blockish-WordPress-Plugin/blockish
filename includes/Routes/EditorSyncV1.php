...
class EditorSyncV1 extends WP_REST_Controller {
	...
	public function check_refresh( WP_REST_Request $request ) {
		...
		if ( ! $post_id ) {
			return rest_ensure_response( array( 'refresh' => false ) );
		}
		...
		if ( $needs_refresh ) {
			// Delete it immediately so we only refresh once per trigger
			delete_transient( $transient_name );
			return rest_ensure_response( array( 'refresh' => true ) );
		}
