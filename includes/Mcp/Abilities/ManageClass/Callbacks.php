...
class Callbacks
{
    ...
    public static function manage_class( $input ): array
    {
        ...
        if ( $action === 'update' ) {
            ...
            ClassPrevious::snapshot_current( ClassPrevious::resolve_parent_id( $post_id ), false );
            $args['ID'] = $post_id;
            $result     = wp_update_post( $args, true );
        } else {
            $parent_id = absint( $args['post_parent'] ?? 0 );
            if ( $parent_id > 0 ) {
                ClassPrevious::snapshot_current( $parent_id, false );
            }
        ...
        if ( 'update' !== $action && (int) get_post_field( 'post_parent', $id ) < 1 ) {
            ClassPrevious::snapshot_current( $id, true );
        }
    ...
    private static function persist_class_tree( string $name, array $content, array $children, string $generated_css, ?int $force_post_id ): array {
        ...
        if ( $existing_id ) {
            ...
            ClassPrevious::snapshot_current( $existing_id, false );
            $args['ID'] = $existing_id;
            $result     = wp_update_post( $args, true );
        } else {
            $result = wp_insert_post( $args, true );
