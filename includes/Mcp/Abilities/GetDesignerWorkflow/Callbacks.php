...
class Callbacks
{
    public static function get_workflow( $_input ): array
    {
        $workflow = [
            ...
            '4. Styles: Class Manager first for visual CSS (colors, type, padding, shadows) — full rules in get-class-manager-docs §2b (attributes vs Class Manager checklist). Structural props (`display`, `containerWidth`, `gridLayoutType`, `tagName`, `align`, `flexDirection` when it drives layout-type classes, content attrs) MUST stay on the block schema — Class Manager alone often loses to stylesheet `layout-type-*`. Order: `get-classes` → `manage-class` `{css}` only → MUST attach `"classManager": "name1, name2"`; nested `:hover`/descendants → `"classManagerSubselector"`. convert-css only for true one-offs.',
    ...
    private static function template_workflow_steps(): array {
        $steps = array(
            '7. Build sections as patterns first (`manage-pattern`). Never a monolithic page/template tree. Large JSON → `schema_url` (or server `schema_file`). Use only real IDs returned from manage-pattern — never invent `ref`. New page: stage all patterns first, then stage the page with refs — do not open every pattern editor; see step 11.',
            '9. Pages (`manage-post`): Assemble with `core/block` refs. Full-bleed: `{"name":"core/block","attributes":{"ref":ID,"align":"full"}}`. Do NOT put header/footer template slots on pages — templates render them. Do NOT put pattern HTML in `post_content`. Do NOT set `attributes.content` on `core/block`. If you later edit a pattern that a live parent already refs, re-stage that parent with its current schema too (Accept surface stays on the page the user cares about).',
            '11. Handoff: AGENT follows `get-automation-guideline` verify loop — open **one** `edit_url` (prefer parent page/template; any editor resolves the whole queue) → waitUntil `domcontentloaded` → wait **10–15s** → visit frontend → screenshot. If not resolved, open editor once more (10–15s) and re-check. When resolved, final FE screenshot → give frontend URL. Then fix any visible design issues and re-verify. Then call `get-ai-preview-pending` — if count > 0, tell the user how many designs are pending and to Accept manually in Settings → AI Preview. Do NOT open every pattern edit_url. Do NOT Accept/Discard via MCP.',
            '12. Undo: live content → `get-revisions` / `restore-revision` confirm:true. Pending neon → Discard, not revisions.',
            '13. Interactions: entrance presets (`inView`/`ready`) over animation CSS. Device hide = `hideOn`. Payload: `interactionData` / manage-interactions — one CSS tween (`action.motion.tweens[0]`: x/y/scale/rotation/opacity, duration in seconds).',
