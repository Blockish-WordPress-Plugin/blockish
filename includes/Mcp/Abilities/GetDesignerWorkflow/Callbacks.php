...
class Callbacks
{
    public static function get_workflow( $_input ): array
    {
        ...
        $workflow = array_merge( $workflow, [
            ...
            '11. Interactions & visibility: Prefer entrance presets (`interactionData` + `action.type:"preset"`, `when.event:"inView"` or `"ready"`) over hand-written animation CSS. Use emit/listen for cross-block signals. Use custom JS only when needed (`ready`/`init` for one-time setup — not DOMContentLoaded). Hide blocks per device with `hideOn`, not customCss. Prefer Class Manager classes as stable selectors.',
        ] );
