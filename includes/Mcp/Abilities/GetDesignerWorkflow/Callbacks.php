...
class Callbacks
{
    public static function get_workflow( $_input ): array
    {
        ...
        $workflow = array_merge( $workflow, [
            ...
            '9. Page assembly (`manage-post` ONLY): First call `blockish/get-posts` to see if the target is empty. Empty = no post_content AND no pending_schema. If empty: pass post_content with ONLY synced pattern-ref comments using real IDs from manage-pattern, e.g. <!-- wp:block {"ref":123} /--> — goes live; share post_url. If not empty (has content OR pending): pass block_schema as lightweight pattern refs only {"name":"core/block","attributes":{"ref":123}} — staged for Accept; share edit_url. Never put full section HTML or hand-built Blockish trees in post_content. CRITICAL: never set attributes.content on core/block (especially not ""). WP expects content to be an overrides object or omitted. This empty→post_content path does NOT apply to manage-template.',
            ...
            '11. Handoff: For empty manage-post assemblies written to post_content, share post_url (live). For any staged block_schema (non-empty page, or any template), STOP after staging — share edit_url, call trigger-refresh, user must Accept. Do NOT auto-accept by default — Accept exists so a bad AI schema cannot destroy a live site.',
