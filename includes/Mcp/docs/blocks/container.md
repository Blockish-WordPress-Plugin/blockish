...
| `autoGridWidth` | Responsive | `{"Desktop":"12rem"}` | Used when `gridLayoutType` = `"auto"` <br>**CSS:** `.{{WRAPPER}}.blockish-container.layout-type-grid.grid-layout-type-auto` -> `grid-template-columns: repeat(auto-fill,minmax(min({{VALUE}},100%),1fr));` |
